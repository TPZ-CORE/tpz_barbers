...
$(function () {
	...
	$("#main").on("click", "#appearance-categories-list-name", function () {
		...
		$.post('http://tpz_barbers/request_groom_categories', JSON.stringify({}));
	...
	$("#main").on("click", "#groom-selected-comps-list-prev", function () {
		...
		if ($type == 'texture_id') {
			...
			if (CURRENT_GROOM_CATEGORY_ITEM < 1 ) {
		...
		$.post("http://tpz_barbers/set_groom_textures", JSON.stringify({
